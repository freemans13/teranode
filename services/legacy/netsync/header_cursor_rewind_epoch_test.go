...
func TestSyncManager_TwoGivenUpBlocksAreAskedForInTheOrderTheyAreNeeded(t *testing.T) {
	...
	t.Logf("DIAG list=%d cursorHeight=%d lowerWindow=%d parked=%d owed=%d avg=%d",
		listLen, startHeader.Value.(*headerNode).height,
		h.sm.settings.Legacy.BlockDownloadLowerWindow, h.sm.blockPark.Bytes(), h.sm.blockDownloads.Len(), h.sm.blockSizeTracker.getAverageSize())
