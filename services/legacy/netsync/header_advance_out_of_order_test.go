...
// list. Without that the anchor stays at the front and nothing ever matches it.
// A harness built the other way looks plausible and reproduces none of the
// behaviour under test, which is how the first version of this file went wrong.
//
...
func buildHeaderRound(t *testing.T, n int) (*SyncManager, []chainhash.Hash) {
	...
	// handleHeadersMsg used to trim the anchor itself on reaching the
	// checkpoint; splicing only lands the headers now, so the removal this
	// harness depends on is explicit here, same as everywhere else in the
	// package that needed the checkpoint's anchor gone before it could assert
	// on advanceHeaderListFor.
...
//
// What that cost on mainnet: the header list's front named a block already
// committed, and the frontier race this codebase used to run asked seven peers
// for it over twenty-eight minutes before the node recovered. That racer is
// gone now — the wanted-range pass makes it unnecessary — but the underlying
// property this test pins, that a header leaves the list whichever order the
// two paths run in, still matters on its own.
func TestSyncManager_ACommittedBlockLeavesTheHeaderListWhateverTheOrder(t *testing.T) {
	...
	require.False(t, sm.headerIsInTheList(second),
		"the second block committed too, so its header must be gone; leaving it makes the front of the list name a block already in the chain")

	require.Equal(t, round[2], sm.frontHash(t),
		"the front should be the oldest block still actually wanted")
}

// TestSyncManager_RemovingAMiddleHeaderDoesNotMoveTheFront checks the other
...
// up, so the front of the list must not move.
func TestSyncManager_RemovingAMiddleHeaderDoesNotMoveTheFront(t *testing.T) {
	sm, round := buildHeaderRound(t, 4)

	require.Equal(t, round[0], sm.frontHash(t), "the front starts on the front block")

	sm.advanceHeaderListFor(round[2])

	require.False(t, sm.headerIsInTheList(round[2]))
	require.Equal(t, round[0], sm.frontHash(t),
		"removing a header from the middle must leave the front on the block still missing")
}
