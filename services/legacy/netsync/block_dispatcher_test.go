...
func TestDispatchBlocks_RepliesOncePerBlockAndDrainsTheBacklog(t *testing.T) {
	...
	// The head asks HandleBlockDirect's two opening questions before it dispatches:
	// the block is not stored, and its parent is, so the block is dispatched alone
	// and never parked.
	blockchainClient.On("GetBlockExists", mock.Anything, mock.Anything).Return(false, nil)
	blockchainClient.On("GetBlockHeader", mock.Anything, mock.Anything).Return(&model.BlockHeader{}, &model.BlockHeaderMeta{Height: 100}, nil)
	...
	for i := 0; i < 4; i++ {
		...
		sm.blockDownloads.Add(p, b.Header.BlockHash())
	}
...
// end to end with the work itself stubbed: a registered peer that owes every block
// in the download ledger, the FSM in catch-up, a chain that holds every parent, and
// the maps the tail writes to.
...
func newLoopHarness(t *testing.T, blocks int) *loopHarness {
	...
	// The head asks HandleBlockDirect's two opening questions before it dispatches.
	// No block is stored, and a parent that is not in flight is looked up here and
	// found stored, so nothing in this harness is ever parked; the park itself is
	// exercised in head_park_test.go.
	client.On("GetBlockExists", mock.Anything, mock.Anything).Return(false, nil)
	client.On("GetBlockHeader", mock.Anything, mock.Anything).Return(&model.BlockHeader{}, &model.BlockHeaderMeta{Height: 100}, nil)
...
// case: the head found this block's parent in flight, so it was neither parked nor
// given a resolved in-flight parent (that is the window route's), and it waited for
// an empty frontier. Its parent failed while it waited. It must still never be
// revalidated, and must get exactly the treatment the head's own cascade branch
// gives.
