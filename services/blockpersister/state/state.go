...
//   - string: the block hash (empty if no blocks persisted)
//   - error: any error encountered
func (s *State) GetLastPersistedBlock() (uint32, string, error) {
	s.fileLock.Lock()
	...
	if err != nil {
		if os.IsNotExist(err) {
			return 0, "", nil // Return zero values if file doesn't exist yet
		}
	...
	hash := string(bytes.TrimSpace(hashBytes))
