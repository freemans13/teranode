...
func (u *Server) processBlockFound(ctx context.Context, hash *chainhash.Hash, peerID, baseURL string, useBlock ...*model.Block) error {
	...
	if unifiedRoute {
		...
		//
		// entry is this branch's quick-window entry: the commit is the window committer's to make
		// when there is one. The accounting below is unaffected by which of the two ways the error
		// came back, because quickValidateBlock returns the window's verdict for THIS block either
		// way.
		qErr := u.blockValidation.quickValidateBlock(ctx, block, peerID, baseURL, entry)
