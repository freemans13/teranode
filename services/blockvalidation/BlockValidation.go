...
func (u *BlockValidation) ValidateBlockWithOptions(ctx context.Context, block *model.Block, baseURL string, opts *ValidateBlockOptions) error {
	...
	return u.runOncePerBlock(blockHash, opts, func(opts *ValidateBlockOptions) error {
		...
		_, err = u.validateBlockSubtrees(ctx, block, opts.PeerID, baseURL)
		if err != nil {
...
func (u *BlockValidation) reValidateBlock(blockData revalidateBlockData) error {
	...
	// The blessed-without-revalidation signal is unused on this path, so it is
	// discarded here.
...
// the UTXO-hash checksum disabled. Selection is purely store-capability driven:
// restricted to stores that honour the fast path (u.utxoStore.SupportsOutpointOnlySpend()
// — postgres/sql today; Aerospike returns false and takes the normal path), and
// restricted to blocks at or below the highest HARDCODED checkpoint. Uses the standard
// chain-config checkpoints (not the catchup override) so it fails safe — never engaging
// above the real checkpoint (spec §2.2, invariant I2).
//
// Nil chain params and missing checkpoints now fail closed, and height 0 is excluded — via the shared model.BelowCheckpoint boundary.
...
//     any WithCreateConflicting validator pass. A block with zero subtrees is
//     trivially blessed-without-revalidation (no pass could run). Callers currently
//     discard this signal; it is retained as part of the CheckBlockSubtrees contract.
