...
func (c *blockValidationAPIClient) ProcessBlockWindow(ctx context.Context, in *ProcessBlockWindowRequest, opts ...grpc.CallOption) (*EmptyMessage, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(EmptyMessage)
	err := c.cc.Invoke(ctx, BlockValidationAPI_ProcessBlockWindow_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _BlockValidationAPI_ProcessBlockWindow_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ProcessBlockWindowRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockValidationAPIServer).ProcessBlockWindow(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: BlockValidationAPI_ProcessBlockWindow_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockValidationAPIServer).ProcessBlockWindow(ctx, req.(*ProcessBlockWindowRequest))
	}
	return interceptor(ctx, in, info, handler)
}
