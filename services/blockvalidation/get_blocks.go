...
func (u *Server) blockWorker(ctx context.Context, workerID int, workQueue <-chan workItem, resultQueue chan<- resultItem,
	...
	for {
		...
		case work, ok := <-workQueue:
			...
			contributingPeers, err := u.fetchBlockSubtreesAdaptive(ctx, work.block, peerID, baseURL)
			if err != nil {
...
func (u *Server) fetchBlockSubtreesAdaptive(gCtx context.Context, block *model.Block, peerID, baseURL string) (map[string]struct{}, error) {
	...
	for i, subtreeHash := range block.Subtrees {
		...
		g.Go(func() error {
			trackPeer := func(servingPeerID string) {
				if servingPeerID != "" {
					peersMu.Lock()
					contributingPeers[servingPeerID] = struct{}{}
					peersMu.Unlock()
	...
	if err := g.Wait(); err != nil {
		return nil, errors.NewServiceError("[catchup:fetchBlockSubtreesAdaptive] Failed to fetch subtrees for block %s", block.Hash().String(), err)
	}
