...
func TestBlockValidationValidateBigSubtree(t *testing.T) {
	...
	subtreeValidation, err := stv.New(context.Background(), ulogger.TestLogger{}, tSettings, subtreeStore, txStore, testStore, validatorClient, blockchainClient, nilConsumer, nilConsumer, nil, nil, nil)
	require.NoError(t, err)
