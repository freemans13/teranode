...
func TestService_processCleanupJob(t *testing.T) {
	t.Run("SuccessfulCleanup", func(t *testing.T) {
		...
		// loggedMessages[1] is "starting cleanup scan"
		// Last message should be completion
		assert.Contains(t, loggedMessages[len(loggedMessages)-1], "cleanup job completed")
	})
...
func TestDeleteTombstoned(t *testing.T) {
	t.Run("SuccessfulDelete", func(t *testing.T) {
		...
		db := NewMockDB()
		db.ExecFunc = func(query string, args ...interface{}) (sql.Result, error) {
			// Child safety query expects 2 parameters: blockHeight and safetyWindow
			assert.Contains(t, query, "DELETE FROM transactions")
			assert.Len(t, args, 2)
			assert.Equal(t, uint32(100), args[0])
			assert.Equal(t, uint32(288), args[1]) // safetyWindow
			return &MockResult{rowsAffected: 5}, nil
		}
