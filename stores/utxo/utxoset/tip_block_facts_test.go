...
// These tests pin cases where a UTXO, or the journal copy of a spent UTXO, ends up carrying
// block facts that are wrong once its membership window has retired. Each one needs a reorg or
// a mempool transaction, so none of them can happen during below-checkpoint sync, and all of
// them can at the tip. They were found by reading the code and are written to fail until the
// retirement stamp, or the writers feeding it, are fixed.
//
...
// TestParentSpentWhileUnconfirmedIsStillAnswerableAfterItsWindowRetires: a mempool transaction
// is mined, and a mempool child spends its only UTXO before the membership window retires.
//
// The spend copies the UTXO into the journal with the UTXO's facts, and a mempool-created UTXO
...
// one of its UTXOs, M is un-mined so the parent goes back to the mempool, and then the child's
// spend is undone.
//
...
// transaction is in the mempool. If the parent is re-mined in another block, the move leaves
...
func TestUnspendOfAnUnminedParentRestoresAnUnconfirmedUTXO(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, identExists(t, s, ctx, parent), "un-mining puts the parent back in the mempool table")

	require.NoError(t, s.Unspend(ctx, spends, false))

	h, b := utxoFacts(t, s, ctx, parent)
	require.Equal(t, int32(0), h, "a restored UTXO of a transaction in the mempool is unconfirmed")
