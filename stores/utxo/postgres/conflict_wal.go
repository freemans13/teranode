...
func (s *Store) BeginConflictIntent(ctx context.Context, intent utxo.ConflictIntent) error {
	...
	const q = `INSERT INTO conflict_intents (intent_id, kind, block_height, block_hash, tx_hashes, started_at)
		   VALUES ($1, $2, $3, $4, $5, $6)
...
func (s *Store) PendingConflictIntents(ctx context.Context) ([]utxo.ConflictIntent, error) {
	rows, err := s.pool.Query(ctx, `SELECT kind, block_height, block_hash, tx_hashes, started_at FROM conflict_intents`)
	if err != nil {
