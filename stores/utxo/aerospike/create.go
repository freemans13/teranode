...
func (s *Store) storeExternallyWithLock(
	...
	if err := batchOperate(batchPolicy, batchRecords); err != nil {
		s.logger.Errorf("[%s] Aerospike BatchOperate FAILED for tx %s: %v", funcName, bItem.txHash, err)
		util.SafeSend[error](bItem.done, errors.NewStorageError("[%s] Aerospike batch operation failed for tx %s", funcName, bItem.txHash.String(), err))
		return
	}
