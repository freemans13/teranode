...
func (s *Store) SpendBatchDirect(ctx context.Context, requests []*utxo.BatchSpendRequest) ([]*utxo.BatchSpendResult, error) {
	...
	s.logger.Debugf("[UTXOStore][logger][SpendBatchDirect] requests %d results %d err %v : %s", len(requests), len(results), err, caller())
...
func (s *Store) CreateBatchDirect(ctx context.Context, requests []*utxo.BatchCreateRequest) ([]*utxo.BatchCreateResult, error) {
	...
	s.logger.Debugf("[UTXOStore][logger][CreateBatchDirect] requests %d results %d err %v : %s", len(requests), len(results), err, caller())
