...
func TestConvertToBUMP(t *testing.T) {
	t.Run("valid merkle proof conversion", func(t *testing.T) {
		...
		// global leaf offset is (subtreeIndex<<2)|txIndexInSubtree = (1<<2)|3 = 7. Sibling offsets are
		// (7>>level)^1. (The earlier expectations used subtree-local offsets, which produced BUMPs the
		...
		assert.Equal(t, uint64(6), level0[0].Offset)       // 7 XOR 1 = 6 (sibling of global offset 7)
		assert.Equal(t, sibling1.String(), level0[0].Hash) // display order (BRC-74)
		assert.False(t, level0[0].TxID)
		assert.Equal(t, uint64(7), level0[1].Offset)     // txid at global offset 7 (odd, appended)
		...
		assert.Equal(t, uint64(2), level1[0].Offset)       // (7>>1) XOR 1 = 3 XOR 1 = 2
		assert.Equal(t, sibling2.String(), level1[0].Hash) // display order (BRC-74)

		// Verify block level
		blockLevel := bump.Path[2]
		assert.Equal(t, 1, len(blockLevel))
		assert.Equal(t, uint64(0), blockLevel[0].Offset)           // (7>>2) XOR 1 = 1 XOR 1 = 0
